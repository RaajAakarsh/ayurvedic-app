      ...
      <div className="medicine-info">
        ...
        <div className="prescription-info">
          {medicine.prescription ? (
            <p>
              <span role="img" aria-label="required">💊</span> Prescription Required
            </p>
          ) : (
            <p>
              <span role="img" aria-label="not-required">🔓</span> Prescription Not Required
            </p>
          )}
        </div>
        {cartItem ? (
          <div className="quantity-controls">
            <button onClick={() => handleQuantityChange(medicine._id, -1)}>-</button>
            <span>{cartItem.quantity}</span>
            <button onClick={() => handleQuantityChange(medicine._id, 1)}>+</button>
          </div>
        ) : (
          <button onClick={() => addToCart(medicine)} className="add-to-cart-btn">Add to Cart</button>
        )}
      </div>
    </div>
  );
